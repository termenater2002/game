...
namespace Unity.DeepPose.Core
{
    ...
    class SequenceTensorInt
    {
        ...
        public SequenceTensorInt(int sequenceLength, int maxSequenceLength) : this(-1, sequenceLength, maxSequenceLength)
        {
        }
        ...
        /// <param name="maxSequenceLength"></param>
        public SequenceTensorInt(int bachSize, int sequenceLength, int maxSequenceLength, int firstSize = -1, int secondSize = -1)
        {
        ...
        public Tensor<int> AllocateTensor()
        {
            var tensor = new Tensor<int>(m_TensorShape, m_Buffer);
            return tensor;
        }
        ...
        public void InsertSample(int sequenceIdx)
        {
            ...
            unsafe
            {
                ...
                {
                    UnsafeUtility.MemMove(dstPtr, srcPtr, sizeof(int) * copyCount);
                }
            }
        ...
        {
            if (sequenceLength == m_SequenceLength)
                return;
        ...
        public int this[int batch, int sequence]
        {
            get
            {
        ...
        public int this[int batch, int sequence, int first]
        {
            get
            {
        ...
        public int this[int batch, int sequence, int first, int second]
        {
            get
            {
