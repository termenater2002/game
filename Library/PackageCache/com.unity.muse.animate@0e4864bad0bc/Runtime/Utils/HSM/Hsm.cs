...
namespace Hsm
{
    ...
    class State
    {
        ...
        public virtual void LateUpdate(float aDeltaTime) { }
        public virtual void Render(float aDeltaTime) { }

        public virtual bool UpdateBaking(float delta)
        {
            return false;
        }
        ...
        public StateType FindOuterState<StateType>() where StateType : State
        {
        ...
        public bool IsInOuterState<StateType>() where StateType : State
        {
        ...
        public StateType FindInnerState<StateType>() where StateType : State
        {
        ...
        public bool IsInInnerState<StateType>() where StateType : State
        {
            return FindInnerState<StateType>() != null;
        }

        public StateType FindImmediateInnerState<StateType>() where StateType : State
        {
        ...
        public bool IsInImmediateInnerState<StateType>() where StateType : State
        {
    ...
    class StateMachine
    {
        ...
        private StateMachineLogger mStateMachineLogger;
        ...
        public void Init(Type aInitialStateType, object aOwner = null)
        {
            ...
            mStateMachineLogger = new StateMachineLogger(aOwner);
        }
        ...
        public void LateUpdate(float aDeltaTime)
        {
            LateUpdateStates(aDeltaTime);
        }
        
        public void Render(float aDeltaTime)
        {
            RenderStates(aDeltaTime);
        }
        ...
        public void ProcessStateTransitions()
        {
            ...
            if (loopCountdown == 0)
            {
                DevLogger.LogError("Infinite loop detected !!!");
            }
        }

        void UpdateStates(float aDeltaTime)
        {
...
#pragma warning restore CS0618*/
            ...
            foreach (State state in mStateStack)
            {
                state.UpdateBaking(aDeltaTime);
            }
        }
        
        void LateUpdateStates(float aDeltaTime)
        {
            foreach (State state in mStateStack)
            {
                state.LateUpdate(aDeltaTime);
            }
        }
        
        void RenderStates(float aDeltaTime)
        {
            foreach (State state in mStateStack)
            {
                state.Render(aDeltaTime);
            }
        }
        ...
        {
            mStateMachineLogger.LogTransition(TraceLevel.Verbose, aStackDepth, "(Push)", aStateType);
        ...
        private void PopStatesFromDepth(int aStartDepthInclusive)
        {
            ...
            for (int depth = endDepth; depth >= aStartDepthInclusive; --depth)
            {
                ...
                mStateMachineLogger.LogTransition(TraceLevel.Verbose, depth, "(Pop)", currState.GetType());
                ExitState(currState);
            }
        ...
        private bool ProcessStateTransitionsOnce()
        {
            ...
            {
                mStateMachineLogger.LogTransition(TraceLevel.Info, 0, new Transition(TransitionType.Inner, mInitialStateType, null).ToString(), mInitialStateType);
...
#pragma warning restore CS0618
                switch (trans.TransitionType)
                {
                    ...
                    case TransitionType.Inner:
                        ...
                        mStateMachineLogger.LogTransition(TraceLevel.Info, currDepth + 1, trans.ToString(), trans.TargetStateType);
                    ...
                    case TransitionType.Sibling:
                        mStateMachineLogger.LogTransition(TraceLevel.Info, currDepth, trans.ToString(), trans.TargetStateType);
