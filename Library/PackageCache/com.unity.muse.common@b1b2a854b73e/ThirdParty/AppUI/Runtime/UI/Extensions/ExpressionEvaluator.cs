...
{
    /// <summary>
    /// A simple expression evaluator that can be used in NumberField and similar components.
    /// </summary>
    ...
    class ExpressionEvaluator
    {
        ...
        /// <returns>True if the expression could be evaluated, false otherwise.</returns>
        internal static bool Evaluate<T>(string expression, out T value)
        {
            value = default;
        ...
        static string[] ExpressionToTokens(string expression, out bool hasVariables)
        {
            ...
            {
                if (IsVariable(resultEntry) || IsDelayedFunction(resultEntry))
                {
                    hasVariables = true;
                    break;
                }
